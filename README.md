...
This project explores a maximum-entropy style model on a discrete state space `1..13`, using a Boltzmann-form distribution:
...
2. Computes entropy `S = -sum(p * log(p))` from `beta` (`entropy_from_beta`).
3. Builds and plots the main relationships:
   - `beta(E)` and `T(E) = 1/beta`
   - `S(beta)`
...
![Maximum-Entropy / Canonical-Form Relationships](figures/entropy-relationships.png)
